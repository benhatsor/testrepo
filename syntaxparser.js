...
tables.forEach(tablee => {
  
  tablee.querySelectorAll('tr').forEach((tr, index) => {
  
    if (index != 0) {
  
      var trContent = tr.children[0].textContent;
  
      if (trContent[trContent.length-1].toUpperCase() != trContent[trContent.length-1]) {
   
        cssProperties.push(tr.children[0].textContent);
